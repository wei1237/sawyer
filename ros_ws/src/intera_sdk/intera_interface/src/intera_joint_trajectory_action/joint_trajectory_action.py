...
class JointTrajectoryActionServer(object):
    def __init__(self, limb, reconfig_server, rate=100.0,
        ...
        # ========== 改动1：仿真模式跳过Cuff初始化 ==========
        # 仿真环境（use_sim_time=true）跳过Cuff接口（硬件依赖）
        if not rospy.get_param('/use_sim_time', False):
            self._cuff = intera_interface.Cuff(limb=limb)
        else:
            self._cuff = None
            rospy.loginfo("⚠️  仿真模式：跳过Cuff接口初始化（无需手动控制功能）")
    ...
    def _command_stop(self, joint_names, joint_angles, start_time, dimensions_dict):
        # ========== 改动2：判断self._cuff是否存在 ==========
        cuff_button_pressed = self._cuff.cuff_button() if self._cuff is not None else False
        if (cuff_button_pressed or self._limb.has_collided() or
    ...
    def _command_joints(self, joint_names, point, start_time, dimensions_dict):
        # ========== 改动3：判断self._cuff是否存在 ==========
        cuff_button_pressed = self._cuff.cuff_button() if self._cuff is not None else False
        if (self._limb.has_collided() or not self.robot_is_enabled()
             or not self._alive or cuff_button_pressed):
