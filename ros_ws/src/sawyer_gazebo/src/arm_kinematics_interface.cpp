...
bool ArmKinematicsInterface::init(ros::NodeHandle& nh, std::string side)
{
  ...
  // 修正定时器周期（100Hz）
  update_timer_ = nh.createTimer(ros::Duration(0.01), &ArmKinematicsInterface::update, this);
  return true;
}
...
bool ArmKinematicsInterface::parseParams(const ros::NodeHandle& nh)
{
  ...
  {
    ROS_FATAL_NAMED("kinematics",
        "No gravity tip name for Kinematic Chain found on parameter server");
    return false;
  }
