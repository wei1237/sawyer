...
#-----------------------------------------------------------------------------#
# 1. 替换为你的Sawyer机械臂实际主机名/IP（必改！）
# 示例：若机械臂IP是192.168.1.100，填这个值
robot_hostname="192.168.137.100"

# 2. 替换为你的电脑实际IP（必改！）
# 示例：若电脑IP是192.168.1.200，填这个值
your_ip="192.168.137.128"

# 3. 替换为你实际安装的ROS版本（必改！）
# 示例：若装的是noetic，填"noetic"；装的是melodic，填"melodic"
ros_version="noetic"
#-----------------------------------------------------------------------------#
...
if [ -n "${1}" ]; then
	if [[ "${1}" == "sim" ]] || [[ "${1}" == "local" ]]; then
		...
		if [[ -z ${your_ip} || "${your_ip}" == "192.168.1.200" ]] && \
...
to the root of your catkin workspace.\n"
	...
	if [ -n ${robot_hostname} ] && \
	[[ "${robot_hostname}" == "192.168.1.100" ]]; then
...
'robot_hostname' variable to reflect your Robot's current hostname.\n"
	...
	if [ -n ${your_ip} ] && [[ "${your_ip}" == "192.168.1.200" ]]; then
