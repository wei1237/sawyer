...
spawn_sdf_model() {
  ...
  echo "      Spawning ${model_name} at x=${x} y=${y} z=${z}"
...
verify_world_launch_has_no_experiment_spawn() {
  local launch_path
  launch_path="$(rospack find sawyer_gazebo)/launch/sawyer_world.launch"

  echo "      Verifying world launch does not auto-spawn experiment objects:"
  echo "      ${launch_path}"
...
for node in root.iter("node"):
    ...
    if (
        "green_insert_cylinder" in text
        or "blue_insert_socket" in text
    ):
        bad.append(text)
...
print(
    "      OK: no active green_insert_cylinder / "
    "blue_insert_socket spawn node."
)
PY
}
...
echo "[8/8] Spawning experiment objects LAST..."
...
# CYL_X=0.60 CYL_Y=-0.10 SOCKET_X=0.70 SOCKET_Y=0.00 ~/ros_ws/start_mt3_sim.sh
CYL_X="${CYL_X:-0.60}"
CYL_Y="${CYL_Y:--0.20}"
CYL_Z="${CYL_Z:-0.4500}"

SOCKET_X="${SOCKET_X:-0.70}"
SOCKET_Y="${SOCKET_Y:-0.10}"
SOCKET_Z="${SOCKET_Z:-0.450}"

# Socket first, then cylinder. The entire robot stack is already live and stationary.
spawn_sdf_model \
  blue_insert_socket \
  "${SAWYER_GAZEBO_PATH}/worlds/blue_insert_socket.sdf" \
  "${SOCKET_X}" "${SOCKET_Y}" "${SOCKET_Z}"

spawn_sdf_model \
  green_insert_cylinder \
  "${SAWYER_GAZEBO_PATH}/worlds/green_insert_cylinder.sdf" \
  "${CYL_X}" "${CYL_Y}" "${CYL_Z}"

# Let the dynamic cylinder settle before perception starts.
sleep 2

echo ""
echo "      Experiment scene ready:"
echo "        blue_insert_socket    = (${SOCKET_X}, ${SOCKET_Y}, ${SOCKET_Z})"
echo "        green_insert_cylinder = (${CYL_X}, ${CYL_Y}, ${CYL_Z})"


echo ""
echo "============================================================"
echo "MT3 Sawyer simulation stack started successfully."
...
echo "  python3 mt3_generalize.py ..."
