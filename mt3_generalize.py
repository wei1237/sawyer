...
TOP_GRASP_PIPELINE_BUILD = "2026-08-20_top_grasp_normal_replay_v26"


def _raw_ros_arg(name, default=""):
    """Read a ROS private remap argument directly from sys.argv before rospy.init_node."""
    prefix = "_%s:=" % name
...
def _dispatch_legacy_task_if_needed():
    """
    Compatibility dispatcher for task-specific pipelines.

    The current mt3_generalize.py remains the normal/top-grasp pipeline.
    Legacy vertical-insertion commands are routed to the dedicated,
    current mt3_cylinder_insert_pipeline.py before MT3Pipeline is created.
    Concise task-profile commands for grasp/place experiments are routed to
    mt3_generalize_top_lift.py, which expands them into the proper current
    task-specific pipeline and ROS private parameters.
    """
    raw_task = (_raw_ros_arg("task", "") or "").strip().lower().replace("-", "_")
    ...
    if raw_task in profile_task_aliases:
        script = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            "mt3_generalize_top_lift.py",
        )

        if not os.path.isfile(script):
            raise RuntimeError(
                "task profile dispatcher not found: %s" % script
            )

        cmd = [sys.executable, script] + sys.argv[1:]

        print(
            "[mt3_generalize] compatibility dispatch: "
            "%s -> %s" % (raw_task, os.path.basename(script))
        )
    ...
    if raw_task not in insert_aliases:
        return False
    ...
    if not os.path.isfile(script):
        raise RuntimeError(
            "vertical_insert pipeline not found: %s" % script
        )

    # _task is consumed by this dispatcher.
    # _print_only is also consumed here.
    # All task-specific ROS private parameters such as _demo_id,
    # _use_demo_replay, _use_grasp_replay, _condition_id and
    # _repeat_id must be passed unchanged to the insertion pipeline.
    child_args = []
    for arg in sys.argv[1:]:
        if arg.startswith("_task:="):
            continue
        if arg.startswith("_print_only:="):
            continue
    ...
    print_only = (_raw_ros_arg("print_only", "false") or "").strip().lower()
    if print_only in ("1", "true", "yes", "on"):
        return True

    # Replace this process with the task-specific pipeline so its exit code,
    # ROS private parameters and terminal output remain authoritative.
    os.execv(sys.executable, cmd)
...
class MT3Pipeline:
    def __init__(self):
        ...
        default_phase = (
            "validation" if str(self.experiment_group).lower() == "top_grasp"
            else "unspecified")
        self.experiment_phase = str(rospy.get_param(
            "~experiment_phase", default_phase)).strip() or default_phase
        self.top_grasp_unified_execution = bool(rospy.get_param(
            "~top_grasp_unified_execution", True))
        # v2 keeps the existing replay transition above the mapped bottleneck.
        # The only new correction is a demo-relative mouth-anchor refinement at
        # this safe transition, before the interaction trajectory starts.
        self.top_grasp_transition_offset_m = float(rospy.get_param(
            "~top_grasp_transition_offset_m", 0.100))
        self.top_grasp_vertical_lift_m = float(rospy.get_param(
            "~top_grasp_vertical_lift_m", 0.060))
        self.top_grasp_replay_chunk_size = int(rospy.get_param(
            "~top_grasp_replay_chunk_size", 12))
        self.top_grasp_replay_min_chunk_size = int(rospy.get_param(
            "~top_grasp_replay_min_chunk_size", 1))
        self.top_grasp_replay_min_fraction = float(rospy.get_param(
            "~top_grasp_replay_min_fraction", 0.999))
        self.top_grasp_adaptive_segmented_replay = bool(rospy.get_param(
            "~top_grasp_adaptive_segmented_replay", True))
        self.top_grasp_bottleneck_anchor_gate_m = float(rospy.get_param(
            "~top_grasp_bottleneck_anchor_gate_m", 0.006))
        if (str(self.experiment_group).lower() == "top_grasp" and
                self.top_grasp_unified_execution):
            # Unified-v2.2 is strictly demo driven: no partial replay rescue and
            # no scripted fallback may be counted as a top-grasp trial. Long
            # dense pre-close replay is always planned in short Cartesian chunks;
            # this changes only the planning backend, not the demonstrated path.
            self.use_demo_replay = True
            self.use_top_grasp_replay = True
            self.use_segmented_replay = True
            self.close_on_replay_blocked = False
        ...
        rospy.loginfo(f"  Experiment group: {self.experiment_group or 'unset'}")
        rospy.loginfo(f"  Experiment phase: {self.experiment_phase}")
        rospy.loginfo(f"  Pipeline build: {TOP_GRASP_PIPELINE_BUILD}")
        if str(self.experiment_group).lower() == "top_grasp":
            rospy.loginfo(
                "  Unified top grasp v2.6 normal replay: %s transition_offset=%.1fmm lift=%.1fmm "
                "adaptive_segmented=%s chunk=%d min_chunk=%d full_fraction=%.3f "
                "anchor_gate=%.1fmm",
                self.top_grasp_unified_execution,
                self.top_grasp_transition_offset_m * 1000.0,
                self.top_grasp_vertical_lift_m * 1000.0,
                self.top_grasp_adaptive_segmented_replay,
                self.top_grasp_replay_chunk_size,
                self.top_grasp_replay_min_chunk_size,
                self.top_grasp_replay_min_fraction,
                self.top_grasp_bottleneck_anchor_gate_m * 1000.0)
        # v2.4 freezes the later-project perception path for grasp experiments:
        # semantic mask + registered PointCloud2 + TF.  PnP(+depth) remains in
        # mt3_perception.py only for legacy/non-grasp callers and is never used
        # as a fallback for top_grasp / rotated_top_grasp.
        self.strict_pointcloud_groups = ("top_grasp", "rotated_top_grasp")
        self.strict_pointcloud_perception = (
            str(self.experiment_group).lower() in self.strict_pointcloud_groups)
        if self.use_perception and self.strict_pointcloud_perception:
            rospy.set_param("~use_pointcloud_pose", True)
            rospy.set_param("~strict_pointcloud_pose", True)
        ...
        rospy.loginfo(
            "  Strict PointCloud2 perception: %s",
            self.strict_pointcloud_perception if self.use_perception else False)
        rospy.loginfo("=" * 60)
        ...
        if self.perception is not None:
            rospy.loginfo(
                "  Perception build: %s | pointcloud=%s strict=%s topic=%s",
                getattr(self.perception, "build_marker", "unknown"),
                getattr(self.perception, "use_pointcloud_pose", False),
                getattr(self.perception, "strict_pointcloud_pose", False),
                getattr(self.perception, "pointcloud_topic", ""))
        self.frame_no = 0
    ...
    def _reset_executor_timing_params(self):
        for name, value in [
                ...
                ("/sawyer_auto_grasp/executor_build_marker", ""),
                ("/sawyer_auto_grasp/bottleneck_alignment_attempted", False),
                ("/sawyer_auto_grasp/bottleneck_alignment_success", False),
                ("/sawyer_auto_grasp/bottleneck_descent_attempted", False),
                ("/sawyer_auto_grasp/bottleneck_descent_success", False),
                ("/sawyer_auto_grasp/replay_to_close_attempted", False),
                ("/sawyer_auto_grasp/replay_to_close_success", False),
                ("/sawyer_auto_grasp/gripper_close_attempted", False),
                ("/sawyer_auto_grasp/gripper_close_command_success", False),
                ("/sawyer_auto_grasp/lift_attempted", False),
                ("/sawyer_auto_grasp/lift_success", False),
                ("/sawyer_auto_grasp/transition_target_hand_xyz", ["", "", ""]),
                ("/sawyer_auto_grasp/transition_actual_hand_xyz", ["", "", ""]),
                ("/sawyer_auto_grasp/transition_actual_mouth_xy", ["", ""]),
                ("/sawyer_auto_grasp/transition_anchor_error_before_xy_m", ""),
                ("/sawyer_auto_grasp/transition_anchor_error_xy_m", ""),
                ("/sawyer_auto_grasp/bottleneck_actual_hand_xyz", ["", "", ""]),
                ("/sawyer_auto_grasp/bottleneck_actual_mouth_xy", ["", ""]),
                ("/sawyer_auto_grasp/bottleneck_anchor_error_xy_m", ""),
                ("/sawyer_auto_grasp/bottleneck_anchor_gate_passed", False),
                ("/sawyer_auto_grasp/bottleneck_anchor_gate_threshold_m", ""),
                ("/sawyer_auto_grasp/demo_bottleneck_top_offset_z", ""),
                ("/sawyer_auto_grasp/live_predicted_bottleneck_z", ""),
                ("/sawyer_auto_grasp/live_actual_bottleneck_z", ""),
                ("/sawyer_auto_grasp/live_bottleneck_z_error_m", ""),
                ("/sawyer_auto_grasp/replay_chunk_size_active", ""),
                ("/sawyer_auto_grasp/replay_min_chunk_size_active", ""),
                ("/sawyer_auto_grasp/replay_min_fraction_active", ""),
                ("/sawyer_auto_grasp/adaptive_segmented_replay_active", False),
                ("/sawyer_auto_grasp/adaptive_replay_split_count", 0),
                ("/sawyer_auto_grasp/adaptive_replay_leaf_count", 0),
                ("/sawyer_auto_grasp/adaptive_replay_progress", ""),
                ("/sawyer_auto_grasp/before_close_planned_hand_xyz", ["", "", ""]),
                ("/sawyer_auto_grasp/before_close_actual_hand_xyz", ["", "", ""]),
                ("/sawyer_auto_grasp/before_close_hand_tracking_error_xyz_m", ""),
                ("/sawyer_auto_grasp/preclose_object_xyz", ["", "", ""]),
                ("/sawyer_auto_grasp/preclose_object_shift_xy_m", ""),
                ("/sawyer_auto_grasp/preclose_object_to_target_error_xy_m", ""),
                ("/sawyer_auto_grasp/before_close_mouth_to_object_error_x_m", ""),
                ("/sawyer_auto_grasp/before_close_mouth_to_object_error_y_m", ""),
                ("/sawyer_auto_grasp/before_close_mouth_to_object_error_xy_m", ""),
                ("/sawyer_auto_grasp/postclose_object_xyz", ["", "", ""]),
                ("/sawyer_auto_grasp/close_object_shift_xy_m", ""),
                ("/sawyer_auto_grasp/post_lift_object_xyz", ["", "", ""]),
                ("/sawyer_auto_grasp/post_lift_object_shift_xy_m_executor", ""),
                ("/sawyer_auto_grasp/demo_bottleneck_hand_offset_xy", ["", ""]),
                ("/sawyer_auto_grasp/demo_bottleneck_mouth_offset_xy", ["", ""]),
                ("/sawyer_auto_grasp/demo_object_mouth_offset_xy", ["", ""]),
                ("/sawyer_auto_grasp/demo_tcp_to_mouth_offset_xy", ["", ""]),
                ("/sawyer_auto_grasp/mapped_bottleneck_hand_xy", ["", ""]),
                ("/sawyer_auto_grasp/mapped_bottleneck_mouth_target_xy", ["", ""]),
                ("/sawyer_auto_grasp/mapped_object_mouth_target_xy", ["", ""]),
                ("/sawyer_auto_grasp/top_grasp_transition_offset_m_active", ""),
                ("/sawyer_auto_grasp/before_close_mouth_center_xy", ["", ""]),
                ("/sawyer_auto_grasp/before_close_mouth_center_xyz", ["", "", ""]),
                ...
                ("/sawyer_auto_grasp/before_close_mouth_z", ""),
                ...
                ("/sawyer_auto_grasp/before_close_live_top_z", ""),
                ("/sawyer_auto_grasp/before_close_mouth_to_live_top_z_m", ""),
                ("/sawyer_auto_grasp/before_close_mouth_to_object_error_z_m", "")]:
            try:
    ...
    def _read_executor_timing_params(self):
        ...
        return {
            ...
            "executor_build_marker": str(rospy.get_param(
                "/sawyer_auto_grasp/executor_build_marker", "")),
            "bottleneck_alignment_attempted": _get_bool(
                "/sawyer_auto_grasp/bottleneck_alignment_attempted"),
            "bottleneck_alignment_success": _get_bool(
                "/sawyer_auto_grasp/bottleneck_alignment_success"),
            "bottleneck_descent_attempted": _get_bool(
                "/sawyer_auto_grasp/bottleneck_descent_attempted"),
            "bottleneck_descent_success": _get_bool(
                "/sawyer_auto_grasp/bottleneck_descent_success"),
            "replay_to_close_attempted": _get_bool(
                "/sawyer_auto_grasp/replay_to_close_attempted"),
            "replay_to_close_success": _get_bool(
                "/sawyer_auto_grasp/replay_to_close_success"),
            "gripper_close_attempted": _get_bool(
                "/sawyer_auto_grasp/gripper_close_attempted"),
            "gripper_close_command_success": _get_bool(
                "/sawyer_auto_grasp/gripper_close_command_success"),
            "lift_attempted": _get_bool(
                "/sawyer_auto_grasp/lift_attempted"),
            "lift_success": _get_bool(
                "/sawyer_auto_grasp/lift_success"),
            "transition_target_hand_xyz": _get_vec(
                "/sawyer_auto_grasp/transition_target_hand_xyz", 3),
            "transition_actual_hand_xyz": _get_vec(
                "/sawyer_auto_grasp/transition_actual_hand_xyz", 3),
            "transition_actual_mouth_xy": _get_vec(
                "/sawyer_auto_grasp/transition_actual_mouth_xy", 2),
            "transition_anchor_error_before_xy_m": _get_float(
                "/sawyer_auto_grasp/transition_anchor_error_before_xy_m"),
            "transition_anchor_error_xy_m": _get_float(
                "/sawyer_auto_grasp/transition_anchor_error_xy_m"),
            "bottleneck_actual_hand_xyz": _get_vec(
                "/sawyer_auto_grasp/bottleneck_actual_hand_xyz", 3),
            "bottleneck_actual_mouth_xy": _get_vec(
                "/sawyer_auto_grasp/bottleneck_actual_mouth_xy", 2),
            "bottleneck_anchor_error_xy_m": _get_float(
                "/sawyer_auto_grasp/bottleneck_anchor_error_xy_m"),
            "bottleneck_anchor_gate_passed": _get_bool(
                "/sawyer_auto_grasp/bottleneck_anchor_gate_passed"),
            "bottleneck_anchor_gate_threshold_m": _get_float(
                "/sawyer_auto_grasp/bottleneck_anchor_gate_threshold_m"),
            "demo_bottleneck_top_offset_z": _get_float(
                "/sawyer_auto_grasp/demo_bottleneck_top_offset_z"),
            "live_predicted_bottleneck_z": _get_float(
                "/sawyer_auto_grasp/live_predicted_bottleneck_z"),
            "live_actual_bottleneck_z": _get_float(
                "/sawyer_auto_grasp/live_actual_bottleneck_z"),
            "live_bottleneck_z_error_m": _get_float(
                "/sawyer_auto_grasp/live_bottleneck_z_error_m"),
            "replay_chunk_size_active": _get_int(
                "/sawyer_auto_grasp/replay_chunk_size_active"),
            "replay_min_chunk_size_active": _get_int(
                "/sawyer_auto_grasp/replay_min_chunk_size_active"),
            "replay_min_fraction_active": _get_float(
                "/sawyer_auto_grasp/replay_min_fraction_active"),
            "adaptive_segmented_replay_active": _get_bool(
                "/sawyer_auto_grasp/adaptive_segmented_replay_active"),
            "adaptive_replay_split_count": _get_int(
                "/sawyer_auto_grasp/adaptive_replay_split_count"),
            "adaptive_replay_leaf_count": _get_int(
                "/sawyer_auto_grasp/adaptive_replay_leaf_count"),
            "adaptive_replay_progress": _get_float(
                "/sawyer_auto_grasp/adaptive_replay_progress"),
            "before_close_planned_hand_xyz": _get_vec(
                "/sawyer_auto_grasp/before_close_planned_hand_xyz", 3),
            "before_close_actual_hand_xyz": _get_vec(
                "/sawyer_auto_grasp/before_close_actual_hand_xyz", 3),
            "before_close_hand_tracking_error_xyz_m": _get_float(
                "/sawyer_auto_grasp/before_close_hand_tracking_error_xyz_m"),
            "preclose_object_xyz": _get_vec(
                "/sawyer_auto_grasp/preclose_object_xyz", 3),
            "preclose_object_shift_xy_m": _get_float(
                "/sawyer_auto_grasp/preclose_object_shift_xy_m"),
            "preclose_object_to_target_error_xy_m": _get_float(
                "/sawyer_auto_grasp/preclose_object_to_target_error_xy_m"),
            "before_close_mouth_to_object_error_x_m": _get_float(
                "/sawyer_auto_grasp/before_close_mouth_to_object_error_x_m"),
            "before_close_mouth_to_object_error_y_m": _get_float(
                "/sawyer_auto_grasp/before_close_mouth_to_object_error_y_m"),
            "before_close_mouth_to_object_error_xy_m": _get_float(
                "/sawyer_auto_grasp/before_close_mouth_to_object_error_xy_m"),
            "postclose_object_xyz": _get_vec(
                "/sawyer_auto_grasp/postclose_object_xyz", 3),
            "close_object_shift_xy_m": _get_float(
                "/sawyer_auto_grasp/close_object_shift_xy_m"),
            "post_lift_object_xyz": _get_vec(
                "/sawyer_auto_grasp/post_lift_object_xyz", 3),
            "post_lift_object_shift_xy_m_executor": _get_float(
                "/sawyer_auto_grasp/post_lift_object_shift_xy_m_executor"),
            "demo_bottleneck_hand_offset_xy": _get_vec(
                "/sawyer_auto_grasp/demo_bottleneck_hand_offset_xy", 2),
            "demo_bottleneck_mouth_offset_xy": _get_vec(
                "/sawyer_auto_grasp/demo_bottleneck_mouth_offset_xy", 2),
            "demo_object_mouth_offset_xy": _get_vec(
                "/sawyer_auto_grasp/demo_object_mouth_offset_xy", 2),
            "demo_tcp_to_mouth_offset_xy": _get_vec(
                "/sawyer_auto_grasp/demo_tcp_to_mouth_offset_xy", 2),
            "mapped_bottleneck_hand_xy": _get_vec(
                "/sawyer_auto_grasp/mapped_bottleneck_hand_xy", 2),
            "mapped_bottleneck_mouth_target_xy": _get_vec(
                "/sawyer_auto_grasp/mapped_bottleneck_mouth_target_xy", 2),
            "mapped_object_mouth_target_xy": _get_vec(
                "/sawyer_auto_grasp/mapped_object_mouth_target_xy", 2),
            "top_grasp_transition_offset_m_active": _get_float(
                "/sawyer_auto_grasp/top_grasp_transition_offset_m_active"),
            "before_close_mouth_center_xy": _get_vec(
                "/sawyer_auto_grasp/before_close_mouth_center_xy", 2),
            "before_close_mouth_center_xyz": _get_vec(
                "/sawyer_auto_grasp/before_close_mouth_center_xyz", 3),
            ...
            "before_close_mouth_y": _get_float(
                "/sawyer_auto_grasp/before_close_mouth_y"),
            "before_close_mouth_z": _get_float(
                "/sawyer_auto_grasp/before_close_mouth_z"),
            ...
            "before_close_mouth_error_xy_m": _get_float(
                "/sawyer_auto_grasp/before_close_mouth_error_xy_m"),
            "before_close_live_top_z": _get_float(
                "/sawyer_auto_grasp/before_close_live_top_z"),
            "before_close_mouth_to_live_top_z_m": _get_float(
                "/sawyer_auto_grasp/before_close_mouth_to_live_top_z_m"),
            "before_close_mouth_to_object_error_z_m": _get_float(
                "/sawyer_auto_grasp/before_close_mouth_to_object_error_z_m"),
        }

        # ── 任务类型判断 ────────────────────────────────────────
    def _demo_identity_and_top_anchor(self, demo_entry, aligned=None):
        """Return immutable demo identity and demo-relative bottleneck mouth geometry."""
        info = {
            "recording_date": "",
            "format": "",
            "sha256": "",
            ...
            "demo_object_size": [],
            "demo_object_top_z": "",
            "demo_preclose_hand_z": "",
            "demo_preclose_clearance_above_top": "",
            "anchor_method": "unavailable",
        }
        if not isinstance(demo_entry, dict):
            return info
        demo_id = str(demo_entry.get("id", "") or "")
        source_path = str(
            demo_entry.get("_recorded_json_path", "") or
            demo_entry.get("_recorded_path", "") or "")
        if not source_path and demo_id:
            candidate = os.path.join(
                os.path.dirname(__file__), "demo_library", "recorded", demo_id + ".json")
            if os.path.isfile(candidate):
                source_path = candidate

        # DemoLibrary may expose only selected metadata plus _recorded_trajectory.
        # For the formal top-grasp identity/anchor, always prefer the exact
        # recorded JSON file when it exists so the SHA and mouth calibration
        # refer to the same immutable demonstration.
        source_demo = demo_entry
        if source_path and os.path.isfile(source_path):
            try:
                ...
                rospy.logwarn("  Recorded demo metadata unavailable: %s", exc)
            try:
                h = hashlib.sha256()
                ...
                rospy.logwarn("  Demo sha256 unavailable: %s", exc)

        info["recording_date"] = str(source_demo.get(
            "recording_date", demo_entry.get("recording_date", "")) or "")
        info["format"] = str(source_demo.get(
            "format", demo_entry.get("format", "")) or "")

        def _xyz_from_demo(demo):
            try:
                raw = (demo.get("object_info", {}) or {}).get("position_base")
                if raw and len(raw) >= 3:
                    return [float(raw[0]), float(raw[1]), float(raw[2])]
            except Exception:
                pass
            try:
                pm = (demo.get("object_pose_base_frame", {}) or {}).get("position_m", {})
                return [float(pm["x"]), float(pm["y"]), float(pm["z"])]
            except Exception:
                return None

        def _size_from_demo(demo):
            candidates = []
            try:
                candidates.append((demo.get("object_info", {}) or {}).get("size_m"))
            except Exception:
                pass
            try:
                candidates.append((demo.get("geometric_features", {}) or {}).get("dimensions_m"))
            except Exception:
                pass
            for raw in candidates:
                try:
                    if raw and len(raw) >= 3:
                        return [abs(float(raw[0])), abs(float(raw[1])), abs(float(raw[2]))]
                except Exception:
                    pass
            return None

        demo_object_xyz = _xyz_from_demo(source_demo)
        demo_object_size = _size_from_demo(source_demo)
        if demo_object_xyz is not None:
            info["demo_object_position"] = [float(v) for v in demo_object_xyz]
        if demo_object_size is not None:
            info["demo_object_size"] = [float(v) for v in demo_object_size]
        if demo_object_xyz is not None and demo_object_size is not None:
            try:
                demo_top_z = float(demo_object_xyz[2]) + float(demo_object_size[2])
                info["demo_object_top_z"] = demo_top_z
                traj = (
                    source_demo.get("trajectory") or
                    demo_entry.get("_recorded_trajectory") or {})
                poses = traj.get("poses", []) or []
                close_i = None
                for i, sample in enumerate(poses):
                    label = sample.get("gripper_next", sample.get("gripper_state", None))
                    try:
                        ...
                        pass
                if close_i is None and poses:
                    z_values = [float((p.get("position") or [0.0, 0.0, 0.0])[2])
                                for p in poses if p.get("position")]
                    if z_values:
                        close_i = z_values.index(min(z_values))
                if close_i is not None and close_i < len(poses):
                    pos = poses[close_i].get("position", [])
                    if pos and len(pos) >= 3:
                        demo_preclose_z = float(pos[2])
                        info["demo_preclose_hand_z"] = demo_preclose_z
                        info["demo_preclose_clearance_above_top"] = (
                            demo_preclose_z - demo_top_z)
            except Exception as exc:
                rospy.logwarn("  Demo top-grasp height anchor unavailable: %s", exc)

        obj_demo = None
        try:
            raw = (source_demo.get("object_info", {}) or {}).get("position_base")
            if raw and len(raw) >= 2:
                obj_demo = [float(raw[0]), float(raw[1])]
        except Exception:
            obj_demo = None
        if obj_demo is None:
            try:
                pm = (source_demo.get("object_pose_base_frame", {}) or {}).get("position_m", {})
                obj_demo = [float(pm["x"]), float(pm["y"])]
            except Exception:
                obj_demo = None

        bn_demo = None
        try:
            pm = (source_demo.get("bottleneck_pose_base_frame", {}) or {}).get("position_m", {})
            bn_demo = [float(pm["x"]), float(pm["y"])]
        except Exception:
            bn_demo = None

        mouth_offset = None
        print("DEBUG SOURCE DEMO ID =", source_demo.get("id"))
        print("DEBUG SOURCE DEMO KEYS =", source_demo.keys())
        calib = source_demo.get("top_grasp_mouth_center_calibration", {}) or {}
        for key in ("used_mouth_offset_xy", "mouth_offset_xy"):
            try:
                raw = calib.get(key)
                if raw and len(raw) >= 2:
                    mouth_offset = [float(raw[0]), float(raw[1])]
                    break
            except Exception:
                pass
        if mouth_offset is None:
            try:
                raw = calib.get("mouth_offset_xyz")
                if raw and len(raw) >= 2:
                    mouth_offset = [float(raw[0]), float(raw[1])]
            except Exception:
                mouth_offset = None
        if mouth_offset is not None:
            info["demo_tcp_to_mouth_offset_xy"] = [
                float(mouth_offset[0]), float(mouth_offset[1])]

        demo_mouth_rel = None
        if obj_demo is not None and bn_demo is not None:
            hand_rel = [bn_demo[0] - obj_demo[0], bn_demo[1] - obj_demo[1]]
            info["demo_bottleneck_hand_offset_xy"] = [float(v) for v in hand_rel]

        if obj_demo is not None:
            try:
                mouth_xyz = calib.get("mouth_center_xyz")
                if mouth_xyz and len(mouth_xyz) >= 2:
                    if len(mouth_xyz) >= 3:
                        info["demo_mouth_center_xyz"] = [
                            float(mouth_xyz[0]),
                            float(mouth_xyz[1]),
                            float(mouth_xyz[2]),
                        ]
                        try:
                            demo_top_for_mouth = float(info.get(
                                "demo_object_top_z",
                                float(demo_object_xyz[2]) + float(demo_object_size[2])))
                            info["demo_mouth_top_offset_z"] = (
                                float(mouth_xyz[2]) - demo_top_for_mouth)
                        except Exception:
                            pass
                    demo_mouth_rel = [
                        float(mouth_xyz[0]) - float(obj_demo[0]),
                        float(mouth_xyz[1]) - float(obj_demo[1]),
                    ]
                    info["demo_object_mouth_offset_xy"] = [
                        float(v) for v in demo_mouth_rel]
                    # Keep the historical field populated for the executor and
                    # older CSV columns.  Its value is now the formal
                    # object-relative mouth anchor for top grasp.
                    info["demo_bottleneck_mouth_offset_xy"] = [
                        float(v) for v in demo_mouth_rel]
                    info["anchor_method"] = (
                        "object_center_plus_recorded_mouth_center")
            except Exception:
                demo_mouth_rel = None

        if demo_mouth_rel is None and obj_demo is not None and bn_demo is not None:
            if mouth_offset is not None:
                demo_mouth_rel = [
                    hand_rel[0] + mouth_offset[0],
                    hand_rel[1] + mouth_offset[1],
                ]
                info["demo_object_mouth_offset_xy"] = [
                    float(v) for v in demo_mouth_rel]
                info["demo_bottleneck_mouth_offset_xy"] = [
                    float(v) for v in demo_mouth_rel]
                info["anchor_method"] = "demo_bottleneck_plus_recorded_mouth_offset"

        if aligned:
            try:
                bn_live = (aligned.get("bottleneck_pose", {}) or {}).get("position", [])
                if bn_live and len(bn_live) >= 2:
                    info["mapped_bottleneck_hand_xy"] = [float(bn_live[0]), float(bn_live[1])]
            except Exception:
                pass
            if demo_mouth_rel is not None:
                try:
                    delta = float((aligned.get("pointcloud_yaw_alignment", {}) or {}).get(
                        "delta_yaw_rad", 0.0) or 0.0)
                    c, sn = math.cos(delta), math.sin(delta)
                    rel_live = [
                        c * demo_mouth_rel[0] - sn * demo_mouth_rel[1],
                        sn * demo_mouth_rel[0] + c * demo_mouth_rel[1],
                    ]
                    obj_live = (aligned.get("object_pose_base", {}) or {}).get("position", [])
                    if obj_live and len(obj_live) >= 2:
                        mapped_mouth_xy = [
                            float(obj_live[0]) + rel_live[0],
                            float(obj_live[1]) + rel_live[1],
                        ]
                        info["mapped_object_mouth_target_xy"] = [
                            float(v) for v in mapped_mouth_xy]
                        # Backward-compatible name used by the executor.
                        info["mapped_bottleneck_mouth_target_xy"] = [
                            float(v) for v in mapped_mouth_xy]
                except Exception as exc:
                    rospy.logwarn("  Demo-relative mouth anchor mapping failed: %s", exc)
        return info
    ...
    def _record_experiment_result(
        ...
        try:
            ...
            demo_identity = self._demo_identity_and_top_anchor(best_demo or {}, aligned)
            height_z_info = (aligned or {}).get("height_aware_top_grasp_mapping", {})
            if top_z_info:
                grasp_z_mapping_method = "pointcloud_top_surface_demo_clearance"
            elif height_z_info:
                grasp_z_mapping_method = "height_aware_object_size"
                top_z_info = height_z_info
            else:
                grasp_z_mapping_method = "unchanged_demo_relative"
            ...
            row = {
                ...
                "experiment_phase": str(self.experiment_phase),
                "pipeline_build_marker": TOP_GRASP_PIPELINE_BUILD,
                "perception_build_marker": getattr(
                    self.perception, "build_marker", "") if self.perception else "",
                "perception_strict_pointcloud": bool(
                    getattr(self.perception, "strict_pointcloud_pose", False))
                    if self.perception else False,
                "perception_pointcloud_topic": getattr(
                    self.perception, "pointcloud_topic", "") if self.perception else "",
                "executor_build_marker": self._timing.get("executor_build_marker", ""),
                "top_grasp_transition_offset_m_active": (
                    float(self.top_grasp_transition_offset_m)
                    if str(self.experiment_group).lower() == "top_grasp" else ""),
                "top_grasp_vertical_lift_m_active": (
                    float(self.top_grasp_vertical_lift_m)
                    if str(self.experiment_group).lower() == "top_grasp" else ""),
                "top_mouth_xy_tolerance_m_active": (
                ...
                "postcheck_mode": postcheck.get("postcheck_mode", ""),
                "post_grasp_absolute_z_check_used": postcheck.get(
                    "post_grasp_absolute_z_check_used", ""),
                "reason": reason,
                ...
                "retrieved_demo_recording_date": demo_identity.get("recording_date", ""),
                "retrieved_demo_format": demo_identity.get("format", ""),
                "retrieved_demo_sha256": demo_identity.get("sha256", ""),
                ...
                "post_lift_object_shift_xy_m": postcheck.get(
                    "post_lift_object_shift_xy_m",
                    self._timing.get("post_lift_object_shift_xy_m_executor", "")),
                "object_retained_after_lift": postcheck.get(
                    "object_retained_after_lift", ""),
                "post_grasp_min_lift_delta_m_active": postcheck.get(
                    "post_grasp_min_lift_delta_m_active",
                    _safe_read_param("~post_grasp_min_lift_delta_m",
                                     rospy.get_param(
                                         "/sawyer_auto_grasp/post_grasp_min_lift_delta_m",
                                         0.030))),
                "post_grasp_max_xy_shift_m_active": postcheck.get(
                    "post_grasp_max_xy_shift_m_active",
                    _safe_read_param("~post_grasp_max_xy_shift_m",
                                     rospy.get_param(
                                         "/sawyer_auto_grasp/post_grasp_max_xy_shift_m",
                                         0.12))),
                ...
                "grasp_z_mapping_method": grasp_z_mapping_method,
                ...
                "bottleneck_alignment_attempted": self._timing.get("bottleneck_alignment_attempted", ""),
                "bottleneck_alignment_success": self._timing.get("bottleneck_alignment_success", ""),
                "bottleneck_descent_attempted": self._timing.get("bottleneck_descent_attempted", ""),
                "bottleneck_descent_success": self._timing.get("bottleneck_descent_success", ""),
                "replay_to_close_attempted": self._timing.get("replay_to_close_attempted", ""),
                "replay_to_close_success": self._timing.get("replay_to_close_success", ""),
                "gripper_close_attempted": self._timing.get("gripper_close_attempted", ""),
                "gripper_close_command_success": self._timing.get("gripper_close_command_success", ""),
                "lift_attempted": self._timing.get("lift_attempted", ""),
                "lift_success": self._timing.get("lift_success", ""),
                "demo_bottleneck_hand_offset_xy": json.dumps(
                    self._timing.get("demo_bottleneck_hand_offset_xy",
                                     demo_identity.get("demo_bottleneck_hand_offset_xy", ["", ""])),
                    ensure_ascii=False),
                "demo_bottleneck_mouth_offset_xy": json.dumps(
                    self._timing.get("demo_bottleneck_mouth_offset_xy",
                                     demo_identity.get("demo_bottleneck_mouth_offset_xy", ["", ""])),
                    ensure_ascii=False),
                "demo_object_mouth_offset_xy": json.dumps(
                    self._timing.get("demo_object_mouth_offset_xy",
                                     demo_identity.get("demo_object_mouth_offset_xy", ["", ""])),
                    ensure_ascii=False),
                "demo_tcp_to_mouth_offset_xy": json.dumps(
                    self._timing.get("demo_tcp_to_mouth_offset_xy",
                                     demo_identity.get("demo_tcp_to_mouth_offset_xy", ["", ""])),
                    ensure_ascii=False),
                "mapped_bottleneck_hand_xy": json.dumps(
                    self._timing.get("mapped_bottleneck_hand_xy",
                                     demo_identity.get("mapped_bottleneck_hand_xy", ["", ""])),
                    ensure_ascii=False),
                "mapped_bottleneck_mouth_target_xy": json.dumps(
                    self._timing.get("mapped_bottleneck_mouth_target_xy",
                                     demo_identity.get("mapped_bottleneck_mouth_target_xy", ["", ""])),
                    ensure_ascii=False),
                "mapped_object_mouth_target_xy": json.dumps(
                    self._timing.get("mapped_object_mouth_target_xy",
                                     demo_identity.get("mapped_object_mouth_target_xy", ["", ""])),
                    ensure_ascii=False),
                "transition_target_hand_xyz": json.dumps(
                    self._timing.get("transition_target_hand_xyz", ["", "", ""]),
                    ensure_ascii=False),
                "transition_actual_hand_xyz": json.dumps(
                    self._timing.get("transition_actual_hand_xyz", ["", "", ""]),
                    ensure_ascii=False),
                "transition_actual_mouth_xy": json.dumps(
                    self._timing.get("transition_actual_mouth_xy", ["", ""]),
                    ensure_ascii=False),
                "transition_anchor_error_before_xy_m": self._timing.get(
                    "transition_anchor_error_before_xy_m", ""),
                "transition_anchor_error_xy_m": self._timing.get(
                    "transition_anchor_error_xy_m", ""),
                "bottleneck_actual_hand_xyz": json.dumps(
                    self._timing.get("bottleneck_actual_hand_xyz", ["", "", ""]),
                    ensure_ascii=False),
                "bottleneck_actual_mouth_xy": json.dumps(
                    self._timing.get("bottleneck_actual_mouth_xy", ["", ""]),
                    ensure_ascii=False),
                "bottleneck_anchor_error_xy_m": self._timing.get(
                    "bottleneck_anchor_error_xy_m", ""),
                "bottleneck_anchor_gate_passed": self._timing.get(
                    "bottleneck_anchor_gate_passed", ""),
                "bottleneck_anchor_gate_threshold_m": self._timing.get(
                    "bottleneck_anchor_gate_threshold_m", ""),
                "demo_bottleneck_top_offset_z": self._timing.get(
                    "demo_bottleneck_top_offset_z", ""),
                "live_predicted_bottleneck_z": self._timing.get(
                    "live_predicted_bottleneck_z", ""),
                "live_actual_bottleneck_z": self._timing.get(
                    "live_actual_bottleneck_z", ""),
                "live_bottleneck_z_error_m": self._timing.get(
                    "live_bottleneck_z_error_m", ""),
                "replay_chunk_size_active": self._timing.get(
                    "replay_chunk_size_active", ""),
                "replay_min_chunk_size_active": self._timing.get(
                    "replay_min_chunk_size_active", ""),
                "replay_min_fraction_active": self._timing.get(
                    "replay_min_fraction_active", ""),
                "adaptive_segmented_replay_active": self._timing.get(
                    "adaptive_segmented_replay_active", ""),
                "adaptive_replay_split_count": self._timing.get(
                    "adaptive_replay_split_count", ""),
                "adaptive_replay_leaf_count": self._timing.get(
                    "adaptive_replay_leaf_count", ""),
                "adaptive_replay_progress": self._timing.get(
                    "adaptive_replay_progress", ""),
                "before_close_planned_hand_xyz": json.dumps(
                    self._timing.get("before_close_planned_hand_xyz", ["", "", ""]),
                    ensure_ascii=False),
                "before_close_actual_hand_xyz": json.dumps(
                    self._timing.get("before_close_actual_hand_xyz", ["", "", ""]),
                    ensure_ascii=False),
                "before_close_hand_tracking_error_xyz_m": self._timing.get(
                    "before_close_hand_tracking_error_xyz_m", ""),
                "preclose_object_xyz": json.dumps(
                    self._timing.get("preclose_object_xyz", ["", "", ""]),
                    ensure_ascii=False),
                "preclose_object_shift_xy_m": self._timing.get(
                    "preclose_object_shift_xy_m", ""),
                "preclose_object_to_target_error_xy_m": self._timing.get(
                    "preclose_object_to_target_error_xy_m", ""),
                "before_close_mouth_to_object_error_x_m": self._timing.get(
                    "before_close_mouth_to_object_error_x_m", ""),
                "before_close_mouth_to_object_error_y_m": self._timing.get(
                    "before_close_mouth_to_object_error_y_m", ""),
                "before_close_mouth_to_object_error_xy_m": self._timing.get(
                    "before_close_mouth_to_object_error_xy_m", ""),
                "postclose_object_xyz": json.dumps(
                    self._timing.get("postclose_object_xyz", ["", "", ""]),
                    ensure_ascii=False),
                "close_object_shift_xy_m": self._timing.get(
                    "close_object_shift_xy_m", ""),
                "post_lift_object_xyz": json.dumps(
                    self._timing.get("post_lift_object_xyz", ["", "", ""]),
                    ensure_ascii=False),
                "before_close_mouth_center_xy": json.dumps(
                    self._timing.get("before_close_mouth_center_xy", ["", ""]),
                    ensure_ascii=False),
                "before_close_mouth_center_xyz": json.dumps(
                    self._timing.get("before_close_mouth_center_xyz", ["", "", ""]),
                    ensure_ascii=False),
                ...
                "before_close_mouth_z": self._timing.get("before_close_mouth_z", ""),
                ...
                "before_close_live_top_z": self._timing.get(
                    "before_close_live_top_z", ""),
                "before_close_mouth_to_live_top_z_m": self._timing.get(
                    "before_close_mouth_to_live_top_z_m", ""),
                "before_close_mouth_to_object_error_z_m": self._timing.get(
    ...
    def _validate_post_grasp_success(self, aligned):
        """Verify grasp success from simulator state without feeding GT into control.

        Unified top-grasp uses a table-height-independent relative criterion:
        the object must rise by at least ``post_grasp_min_lift_delta_m`` and
        must not undergo an implausibly large XY displacement.  The legacy
        absolute world-Z criterion is retained only for non-unified/rotated
        grasp paths so this change does not silently alter those experiments.
        """
        ...
        if not gazebo_pose:
            ...
            self.last_postcheck_info = {
                ...
                "postcheck_mode": "relative_lift_retention_v23" if group == "top_grasp" else "legacy_absolute_z",
                "post_grasp_absolute_z_check_used": False if group == "top_grasp" else True,
            }
            return True
        ...
        if self.pre_execution_gazebo_pose:
            try:
                ...
                lift_delta = ""
            try:
                dx0 = float(gazebo_pose["x"]) - float(self.pre_execution_gazebo_pose["x"])
                dy0 = float(gazebo_pose["y"]) - float(self.pre_execution_gazebo_pose["y"])
                post_lift_object_shift_xy_m = math.sqrt(dx0 * dx0 + dy0 * dy0)
            except Exception:
                post_lift_object_shift_xy_m = ""

        # ------------------------------------------------------------------
        # Unified top-grasp v2.3 verification: RELATIVE lift + retention.
        # Do not use a fixed world-Z threshold because the top-grasp table
        # surface (~0.325 m) differs from insertion/placement scenes.
        # ------------------------------------------------------------------
        if group == "top_grasp":
            if not self.pre_execution_gazebo_pose:
                rospy.logwarn(
                    "POST-GRASP RELATIVE CHECK: initial Gazebo pose unavailable; "
                    "keeping executor result and marking postcheck unavailable.")
                self.last_postcheck_info = {
                    "postcheck_success": "",
                    "postcheck_reason": "initial_gazebo_pose_unavailable",
                    "postcheck_mode": "relative_lift_retention_v23",
                    "post_grasp_absolute_z_check_used": False,
                    "final_target_error_xy_m": xy_shift,
                    "final_lift_delta_m": lift_delta,
                    "post_lift_object_shift_xy_m": post_lift_object_shift_xy_m,
                    "object_retained_after_lift": "",
                    "post_grasp_min_lift_delta_m_active": min_lift_delta,
                    "post_grasp_max_xy_shift_m_active": max_xy_shift,
                }
                return True

            lift_ok = (lift_delta != "" and float(lift_delta) >= min_lift_delta)
            retention_xy_ok = (
                post_lift_object_shift_xy_m == "" or
                float(post_lift_object_shift_xy_m) <= max_xy_shift)
            object_retained_after_lift = bool(lift_ok and retention_xy_ok)

            rospy.loginfo(
                "POST-GRASP RELATIVE CHECK: model=%s final_z=%.3f "
                "lift_delta=%s min_lift=%.3f post_lift_xy_shift=%s "
                "max_xy=%.3f lift_ok=%s retention_xy_ok=%s retained=%s "
                "absolute_z_check_used=False",
                gazebo_pose.get("name", ""),
                final_z,
                ("%.3f" % lift_delta if lift_delta != "" else "n/a"),
                min_lift_delta,
                ("%.3f" % post_lift_object_shift_xy_m
                 if post_lift_object_shift_xy_m != "" else "n/a"),
                max_xy_shift,
                lift_ok, retention_xy_ok, object_retained_after_lift)

            if object_retained_after_lift:
                self.last_postcheck_info = {
                    "postcheck_success": True,
                    "postcheck_reason": "accepted_with_relative_lift_retention",
                    "postcheck_mode": "relative_lift_retention_v23",
                    "post_grasp_absolute_z_check_used": False,
                    "final_target_error_xy_m": xy_shift,
                    "final_lift_delta_m": lift_delta,
                    "post_lift_object_shift_xy_m": post_lift_object_shift_xy_m,
                    "object_retained_after_lift": True,
                    "post_grasp_min_lift_delta_m_active": min_lift_delta,
                    "post_grasp_max_xy_shift_m_active": max_xy_shift,
                }
                return True
            ...
            if not lift_ok:
                self.last_execution_failure_reason = "抓取后目标物体相对抬升不足"
            else:
                self.last_execution_failure_reason = "抓取后目标物体发生明显XY位移"
            self.last_postcheck_info = {
            ...
            rospy.logerr(
                "POST-GRASP RELATIVE CHECK: rejecting executor success: %s",
                self.last_execution_failure_reason)
            return False

        # ------------------------------------------------------------------
        # Legacy rotated-top path: preserve the previous absolute-Z behavior.
        # ------------------------------------------------------------------
        ...
        object_retained_after_lift = bool(xy_ok and (z_ok or z_margin_ok or lift_ok))
        rospy.loginfo(
        ...
        if xy_ok and (z_ok or z_margin_ok or lift_ok):
            self.last_postcheck_info = {
                ...
                "postcheck_mode": "legacy_absolute_z",
                "post_grasp_absolute_z_check_used": True,
                "final_target_error_xy_m": xy_shift,
                "final_lift_delta_m": lift_delta,
                "post_lift_object_shift_xy_m": post_lift_object_shift_xy_m,
    ...
    def _validate_perception_for_execution(self, test_data, aligned):
        ...
        group = str(getattr(self, "experiment_group", "") or "").lower()
        strict_pc = bool(
            getattr(self, "strict_pointcloud_perception", False) or
            getattr(self.perception, "strict_pointcloud_pose", False)
            if self.perception is not None else False)
        if self.use_perception and strict_pc:
            if method != "LangSAM+depth_pointcloud":
                rospy.logerr(
                    "PERCEPTION SAFETY GATE: rejected pose method=%s for %s. "
                    "Expected strict LangSAM+depth_pointcloud; no PnP fallback is allowed.",
                    method, group or "grasp")
                return False
        elif self.use_perception and self.require_depth_pose and "depth" not in method:
            rospy.logerr(
                f"PERCEPTION SAFETY GATE: rejected pose method={method}. "
                "Expected a depth-backed pose."
            )
            return False
    ...
    def step1_load_test_image(self):
        ...
        if getattr(self.perception, "use_pointcloud_pose", False):
            ...
            if self.perception.head_points is None:
                if getattr(self.perception, "strict_pointcloud_pose", False):
                    rospy.logerr(
                        "  STRICT POINTCLOUD PERCEPTION: no PointCloud2 message; aborting trial")
                    return {
                        ...
                        "pose": {
                            ...
                            "confidence": 0.0,
                            "method": "LangSAM+depth_pointcloud_failed",
                        },
                        "_perception_failed": True,
                        "_perception_failure_reason": "pointcloud_unavailable",
                    }
        ...
        if pose is None:
            if getattr(self.perception, "strict_pointcloud_pose", False):
                ...
                return {
                    ...
                    "_perception_failed": True,
                    "_perception_failure_reason": "pointcloud_pose_failed",
                }
    ...
    def _icp_object_pose(self, icp_result, fallback_pose):
        ...
        try:
            ...
            live_source_frame = self._package_source_frame(icp_result["live_package"])
            live_size = self._estimate_object_size_from_pointcloud(
                live_points, source_frame=live_source_frame)
            ...
            if live_size is not None:
                ...
                pose["estimated_object_size_source"] = "base_pointcloud_p10_p90"
            rospy.loginfo(
    ...
    def _estimate_object_size_from_pointcloud(self, points, source_frame=None):
        ...
        if source_frame:
            ...
            if base_points is not None and len(base_points) >= 10:
                points = base_points
    ...
    def _apply_height_aware_top_grasp(self, aligned):
        ...
        aligned["height_aware_top_grasp_mapping"] = {
            "demo_top_z": "",
            "live_top_z": "",
            "demo_grasp_z": "",
            "demo_clearance_above_top": "",
            "old_grasp_z": old_z,
            "mapped_grasp_z": float(grasp_pos[2]),
            "object_height": float(height),
            "configured_clearance": float(clearance),
        }
    ...
    def step7_execute(self, aligned):
        ...
        is_unified_top = (
            str(self.experiment_group).lower() == "top_grasp" and
            bool(self.top_grasp_unified_execution))
        rospy.set_param('/sawyer_auto_grasp/top_grasp_unified_execution',
                        bool(is_unified_top))
        rospy.set_param('/sawyer_auto_grasp/top_grasp_transition_offset_m',
                        float(self.top_grasp_transition_offset_m))
        rospy.set_param('/sawyer_auto_grasp/top_grasp_vertical_lift_m',
                        float(self.top_grasp_vertical_lift_m))
        rospy.set_param('/sawyer_auto_grasp/top_grasp_bottleneck_anchor_gate_m',
                        float(self.top_grasp_bottleneck_anchor_gate_m))
        rospy.set_param('/sawyer_auto_grasp/replay_chunk_size',
                        int(self.top_grasp_replay_chunk_size))
        rospy.set_param('/sawyer_auto_grasp/replay_min_chunk_size',
                        int(self.top_grasp_replay_min_chunk_size))
        rospy.set_param('/sawyer_auto_grasp/replay_min_fraction',
                        float(self.top_grasp_replay_min_fraction))
        rospy.set_param('/sawyer_auto_grasp/adaptive_segmented_replay',
                        bool(self.top_grasp_adaptive_segmented_replay if is_unified_top else False))
        rospy.set_param('/sawyer_auto_grasp/pipeline_build_marker',
                        TOP_GRASP_PIPELINE_BUILD)
        ...
        if self.pre_execution_gazebo_pose:
            rospy.set_param(
                '/sawyer_auto_grasp/gazebo_target_model_name',
                str(self.pre_execution_gazebo_pose.get('name', '')))
            rospy.set_param(
                '/sawyer_auto_grasp/initial_object_xyz_world',
                [float(self.pre_execution_gazebo_pose.get('x', 0.0)),
                 float(self.pre_execution_gazebo_pose.get('y', 0.0)),
                 float(self.pre_execution_gazebo_pose.get('z', 0.0))])
        else:
            rospy.set_param('/sawyer_auto_grasp/gazebo_target_model_name', '')
            rospy.set_param('/sawyer_auto_grasp/initial_object_xyz_world', [])
        ...
        rospy.set_param('/sawyer_auto_grasp/replay_chunk_size_active',
                        int(self.top_grasp_replay_chunk_size) if is_unified_top else "")
        ...
        if is_unified_top:
            # Formal top-grasp method: no partial-replay rescue, no scripted
            # fallback, and no low-height correction after replay begins.
            rospy.set_param('/sawyer_auto_grasp/close_on_replay_blocked', False)
            rospy.set_param('/sawyer_auto_grasp/fallback_to_scripted_grasp', False)
            rospy.set_param('/sawyer_auto_grasp/use_top_mouth_center_final_correction', False)
            rospy.set_param('/sawyer_auto_grasp/use_top_mouth_anchor_refinement', True)
            rospy.set_param('/sawyer_auto_grasp/debug_locked_xy_descent', True)
        ...
        if self.use_demo_replay:
            ...
            demo_identity = self._demo_identity_and_top_anchor(demo_entry, aligned)
            if demo_traj:
                self.last_demo_replay_path = os.path.join(
                    self.rollout_trajectory_dir,
                    "replay_input_%s.json" % self.trial_id)
                demo_object_position = demo_identity.get(
                    "demo_object_position", []) or []
                demo_object_size = demo_identity.get("demo_object_size", []) or []
                if len(demo_object_position) < 3:
                    try:
                        raw = (demo_entry.get("object_info", {}) or {}).get(
                            "position_base")
                        if raw and len(raw) >= 3:
                            demo_object_position = [float(v) for v in raw[:3]]
                    except Exception:
                        pass
                if len(demo_object_size) < 3:
                    try:
                        raw = (demo_entry.get("object_info", {}) or {}).get(
                            "size_m")
                        if raw and len(raw) >= 3:
                            demo_object_size = [abs(float(v)) for v in raw[:3]]
                    except Exception:
                        pass
                if is_unified_top and (
                        len(demo_object_position) < 3 or
                        len(demo_object_size) < 3 or
                        len(obj_pos) < 3 or
                        len(execution_object_size) < 3):
                    rospy.logerr(
                        "  TOP UNIFIED replay input missing object-relative "
                        "anchors: demo_object_position=%s demo_object_size=%s "
                        "live_object_position=%s live_object_size=%s",
                        demo_object_position, demo_object_size,
                        obj_pos, execution_object_size)
                    self.last_execution_failure_stage = "demo_replay_input"
                    self.last_execution_failure_reason = (
                        "top_grasp_object_relative_anchor_missing")
                    return False
                replay_payload = {
                    "format": "mt3_demo_replay_input_v1",
                    "source_demo": demo_entry.get("id", aligned.get("retrieved_demo_name", "")),
                    "source_demo_recording_date": demo_identity.get("recording_date", ""),
                    "source_demo_format": demo_identity.get("format", ""),
                    "source_demo_sha256": demo_identity.get("sha256", ""),
                    ...
                    "demo_bottleneck_hand_offset_xy": demo_identity.get(
                        "demo_bottleneck_hand_offset_xy", ["", ""]),
                    "demo_bottleneck_mouth_offset_xy": demo_identity.get(
                        "demo_bottleneck_mouth_offset_xy", ["", ""]),
                    "demo_object_mouth_offset_xy": demo_identity.get(
                        "demo_object_mouth_offset_xy", ["", ""]),
                    "demo_mouth_center_xyz": demo_identity.get(
                        "demo_mouth_center_xyz", ["", "", ""]),
                    "demo_mouth_top_offset_z": demo_identity.get(
                        "demo_mouth_top_offset_z", ""),
                    "demo_tcp_to_mouth_offset_xy": demo_identity.get(
                        "demo_tcp_to_mouth_offset_xy", ["", ""]),
                    "mapped_bottleneck_hand_xy": demo_identity.get(
                        "mapped_bottleneck_hand_xy", ["", ""]),
                    "mapped_bottleneck_mouth_target_xy": demo_identity.get(
                        "mapped_bottleneck_mouth_target_xy", ["", ""]),
                    "mapped_object_mouth_target_xy": demo_identity.get(
                        "mapped_object_mouth_target_xy", ["", ""]),
                    "demo_object_position": [float(v) for v in demo_object_position[:3]],
                    "demo_object_size": [float(v) for v in demo_object_size[:3]],
                    "live_object_position": [float(v) for v in obj_pos[:3]],
                    "live_object_size": [float(v) for v in execution_object_size[:3]],
                    "demo_object_top_z": demo_identity.get("demo_object_top_z", ""),
                    "demo_preclose_hand_z": demo_identity.get("demo_preclose_hand_z", ""),
                    "demo_preclose_clearance_above_top": demo_identity.get(
                        "demo_preclose_clearance_above_top", ""),
                    "tail_correction_points": int(rospy.get_param(
                        "~top_grasp_tail_correction_points", 10)),
                    "top_grasp_anchor_method": demo_identity.get("anchor_method", "unavailable"),
                }
                rospy.set_param('/sawyer_auto_grasp/demo_bottleneck_hand_offset_xy',
                                replay_payload.get('demo_bottleneck_hand_offset_xy', ["", ""]))
                rospy.set_param('/sawyer_auto_grasp/demo_bottleneck_mouth_offset_xy',
                                replay_payload.get('demo_bottleneck_mouth_offset_xy', ["", ""]))
                rospy.set_param('/sawyer_auto_grasp/demo_object_mouth_offset_xy',
                                replay_payload.get('demo_object_mouth_offset_xy', ["", ""]))
                rospy.set_param('/sawyer_auto_grasp/demo_tcp_to_mouth_offset_xy',
                                replay_payload.get('demo_tcp_to_mouth_offset_xy', ["", ""]))
                rospy.set_param('/sawyer_auto_grasp/mapped_bottleneck_hand_xy',
                                replay_payload.get('mapped_bottleneck_hand_xy', ["", ""]))
                rospy.set_param('/sawyer_auto_grasp/mapped_bottleneck_mouth_target_xy',
                                replay_payload.get('mapped_bottleneck_mouth_target_xy', ["", ""]))
                rospy.set_param('/sawyer_auto_grasp/mapped_object_mouth_target_xy',
                                replay_payload.get('mapped_object_mouth_target_xy', ["", ""]))
                rospy.set_param('/sawyer_auto_grasp/top_grasp_transition_offset_m_active',
                                float(self.top_grasp_transition_offset_m) if is_unified_top else "")
                ...
                rospy.loginfo(
                    "  Top replay object anchors: demo_obj=%s demo_size=%s "
                    "live_obj=%s live_size=%s",
                    replay_payload.get("demo_object_position"),
                    replay_payload.get("demo_object_size"),
                    replay_payload.get("live_object_position"),
                    replay_payload.get("live_object_size"))
            else:
                if is_unified_top:
                    rospy.logerr(
                        "  TOP UNIFIED v2 requires a recorded demo trajectory; "
                        "scripted fallback is forbidden.")
                    self.last_execution_failure_stage = "demo_replay_input"
                    self.last_execution_failure_reason = "top_grasp_demo_trajectory_missing"
        ...
        try:
            ...
            error_patterns = [
                (
                    "top unified bottleneck alignment failed",
                    "bottleneck_alignment",
                    "top_unified_bottleneck_alignment_failed",
                ),
                (
                    "top unified bottleneck descent failed",
                    "bottleneck_descent",
                    "top_unified_bottleneck_descent_failed",
                ),
                (
                    "top unified replay to close failed",
                    "replay_to_close",
                    "top_unified_replay_to_close_failed",
                ),
                (
                    "top unified gripper close failed",
                    "gripper_close",
                    "top_unified_gripper_close_failed",
                ),
                (
                    "top unified lift failed",
                    "lift",
                    "top_unified_lift_failed",
                ),
                (
    ...
    def run(self):
        ...
        if (test_data or {}).get("_perception_failed", False):
            reason = str((test_data or {}).get(
                "_perception_failure_reason", "pointcloud_pose_failed"))
            self.last_execution_failure_stage = "perception"
            self.last_execution_failure_reason = reason
            rospy.logerr(
                "MT3 Pipeline: ABORTED by strict PointCloud2 perception (%s).",
                reason)
            self.last_executor_success = False
            self._record_experiment_result(
                "failed", test_data=test_data, reason=reason)
            return False
...
if __name__ == "__main__":
    try:
        if _dispatch_legacy_task_if_needed():
            sys.exit(0)
