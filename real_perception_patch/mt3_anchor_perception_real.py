#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Real Sawyer two-object perception for anchored pick-and-place.

This real-only module keeps the existing anchor geometry logic but replaces the
old head-camera/organized-registered-depth point cloud input with ASC60C registered depth +
CameraInfo + external LangSAM masks.  It detects:
...
from mt3_alignment_real import TrajectoryAligner, pose_compose, quat_rotate
from mt3_perception_real import PerceptionNode


DEFAULT_TARGET_MASK = "/mnt/hgfs2/ascamera_data/current_mask.npy"
DEFAULT_ANCHOR_MASK = "/mnt/hgfs2/ascamera_data/current_anchor_mask.npy"
...
class DualMaskAnchorPerception(object):
    """Detect target and anchor from two masks plus registered ASC60C depth."""
    ...
    def wait_for_pointcloud(self, timeout_s=8.0):
        """Compatibility name: real mode waits for registered RGB-D + CameraInfo."""
        ok = self.perception.wait_for_registered_rgbd(timeout_s=timeout_s)
        if not ok:
            rospy.logwarn(
                "[AnchorPerceptionReal] no registered ASC60C RGB-D/CameraInfo within %.1fs",
                timeout_s)
        return ok

    def _detect_one(self, label, mask_path):
        if not os.path.exists(mask_path):
            rospy.logwarn("[AnchorPerceptionReal] %s mask missing: %s", label, mask_path)
            return None
        old_path = self.perception.langsam_mask_path
        try:
            self.perception.langsam_mask_path = mask_path
            pose_source = self.perception.estimate_pose_with_depth_mask(mask_path)
        finally:
            self.perception.langsam_mask_path = old_path
        if pose_source is None:
            rospy.logwarn("[AnchorPerceptionReal] %s detection failed", label)
            return None
        ...
        if pose_base is None:
            rospy.logwarn("[AnchorPerceptionReal] %s TF to base failed", label)
            return None
        ...
        return {
            ...
            "method": "dual_LangSAM_masks+registered_depth+CameraInfo+calibrated_TF",
        }
    ...
    def _current_scene_arrays(self):
        ...
        try:
            ...
            rospy.logwarn("[AnchorPerceptionReal] RGB snapshot unavailable: %s", exc)
        ...
        try:
            ...
            rospy.logwarn("[AnchorPerceptionReal] depth snapshot unavailable: %s", exc)
        ...
        try:
            ...
            rospy.logwarn(
                "[AnchorPerceptionReal] camera intrinsics unavailable: %s", exc)
    ...
    def _points_to_base(self, pose_source):
        ...
        try:
            ...
            rospy.logwarn("[AnchorPerceptionReal] geometry correction TF failed: %s",
                          exc)
            return None
    ...
    def _camera_points_to_base(self, points, source_frame):
        ...
        try:
            ...
            rospy.logwarn("[AnchorPerceptionReal] camera point TF failed: %s", exc)
            return None

    def _anchor_hole_center_from_mask_center(self, pose_source):
        """Use a registered-depth patch around the mask center on the real camera."""
        center = pose_source.get("mask_center_2d")
        source_frame = pose_source.get("source_frame")
        if center is None or source_frame is None:
            return None
        try:
            radius = int(rospy.get_param("~anchor_center_depth_patch_px", 4))
            points = self.perception.camera_points_from_depth_patch(
                center, radius_px=radius)
            ...
            rospy.logwarn(
                "[AnchorPerceptionReal] anchor mask-center registered-depth failed: %s",
                exc)
            return None
    ...
    def _camera_intrinsics(self):
        values = self.perception.get_camera_intrinsics_tuple()
        if values is None:
            raise RuntimeError(
                "ASC60C CameraInfo unavailable; real mode forbids Gazebo intrinsics fallback")
        return values
    ...
    def _source_frame_tf(self, source_frame):
        ...
        try:
            ...
            rospy.logwarn("[AnchorPerceptionReal] source-frame TF failed: %s", exc)
            return None
    ...
    def _anchor_center_from_mask_ray_plane(self, pose_source, z_plane):
        ...
        try:
            ...
            rospy.logwarn("[AnchorPerceptionReal] mask ray-plane center failed: %s",
                          exc)
            return None
    ...
    def _project_image_pixel_to_plane(self, center, source_frame, z_plane):
        ...
        try:
            ...
            rospy.logwarn("[AnchorPerceptionReal] image ray-plane projection failed: %s",
                          exc)
            return None
    ...
    def _latest_bgr_image(self):
        try:
            ...
            rospy.logwarn("[AnchorPerceptionReal] BGR image unavailable: %s", exc)
            return None
    ...
    def _load_pose_mask(self, pose_source):
        ...
        try:
            ...
            rospy.logwarn("[AnchorPerceptionReal] load anchor mask failed: %s", exc)
            return None
    ...
    def _fit_mask_plane_circle(self, pixels, pose_source, z_plane,
        ...
        try:
            ...
            rospy.logwarn("[AnchorPerceptionReal] %s failed: %s", method, exc)
            return None
    ...
    def _correct_geometry_center(self, label, pose_source, pose_base):
        ...
        if label == "anchor":
            ...
            if dark_hole is not None and ray_center is not None:
                ...
                if dark_delta > max_dark_delta:
                    rospy.logwarn(
                        "[AnchorPerceptionReal] rejecting dark hole center: "
            ...
            if (dark_hole is not None and
                ...
                if ray_center is not None:
                    ...
                    if fit_delta > max_fit_delta:
                        rospy.logwarn(
                            "[AnchorPerceptionReal] rejecting %s: delta from "
        ...
        rospy.loginfo(
            "[AnchorPerceptionReal] %s geometry center correction: "
    ...
    def detect_scene(self, timeout_s=8.0):
        ...
        if target is None or anchor is None:
            return None
        rospy.loginfo(
            "[AnchorPerceptionReal] target_base=[%.3f %.3f %.3f] "
